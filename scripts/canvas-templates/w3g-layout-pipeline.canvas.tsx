...
const SCROLL_ID = "w3g-pipeline-scroll";
...
function StageDiagram({
  ...
  const [zoom, setZoom] = useCanvasState(`w3g-pipe-zoom-${stage.step}`, 0.25);
...
export default function W3gLayoutPipelineCanvas() {
  const theme = useHostTheme();
  const payload = PIPELINE as PipelinePayload;
  const [stepIndex, setStepIndex] = useCanvasState("w3g-pipe-step", 0);
  ...
  return (
    <Stack gap={20} style={{ padding: 24, maxWidth: "100%" }}>
      <Stack gap={6}>
        ...
        <Text tone="secondary">
          Step through column-layout phases for {payload.meta.scenarioCode}. Person positions after each phase;
          metrics show engine width and excess cousin gap (slack above FAMILY_GAP).
        </Text>
      </Stack>

      <Callout tone="info" title="How to use">
        Use Previous / Next or the step dropdown to walk the pipeline in order. You can also click
        any row in the list below. Watch when max cousin slack jumps — that is when the empty band
        appears or grows. Scroll to pan; +/− to zoom.
      </Callout>

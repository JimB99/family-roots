...
const HIGHLIGHT_IDS = new Set([
  ...
  "c2rhuEFVsjPMOFcbG9lB",
  "BICMSUdMZwUdHVwofT2n",
]);
...
function LayoutSvg({
  ...
  const fontSize = Math.max(5, Math.min(10, 8 * scale));
        ...
        if (e.type === "spouse" && a.y === b.y) {
          ...
          return (
            <line
              ...
              strokeWidth={Math.max(0.75, 1.2 * scale)}
            />
          );
        }
        return (
          <line
            ...
            strokeWidth={Math.max(0.5, scale * 0.75)}
            opacity={0.4}
          />
        );
      })}
...
function StageDiagram({
  ...
  const fitWidth = () => {
    ...
    setZoom(Math.max(0.02, Math.min(1.25, viewW / srcW)));
  };
  ...
  return (
    <Stack gap={20} style={{ padding: 24, maxWidth: "100%" }}>
      ...
      <Callout tone="info" title="How to use">
        Use Previous / Next or the dropdown. For join trees, watch joinPackDescendants and
        joinBindingRow beside the cross-marriage hub. Fit width helps on this wide pedigree. Pan by scrolling.
      </Callout>

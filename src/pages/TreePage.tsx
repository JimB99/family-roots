...
import { TreeEditPanel, TreeLinkLegend } from '../components/TreeEditPanel'
  ...
  return (
    <Layout familyName={family.name} slug={slug} isEditor={isEditor} adminHref={`/families/${slug}/admin`}>
      <div className="flex flex-col h-[calc(100svh-3.5rem)] min-h-0">
        ...
        {viewMode === 'branch' && (
          <div className="border-b border-stone-200 bg-white px-4 py-2 shrink-0 space-y-2">
            <TreeLinkLegend />
            {editMode && isEditor && (
              <TreeEditPanel
                selectedPerson={selectedPerson}
                onAddChild={() =>
                  selectedPersonId && chartApi && startAddChild(chartApi.chart, selectedPersonId)
                }
                onAddSpouse={() =>
                  selectedPersonId && chartApi && startAddSpouse(chartApi.chart, selectedPersonId)
                }
                onAddParent={() =>
                  selectedPersonId && chartApi && startAddParent(chartApi.chart, selectedPersonId)
                }
                onAddSibling={() =>
                  selectedPersonId && chartApi && startAddSibling(chartApi.chart, selectedPersonId)
                }
        ...
        <div className="relative flex-1 min-h-0 min-h-[480px]">
          {viewMode === 'branch' ? (
            <FamilyTree
              people={people}
